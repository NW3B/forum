...
if(!isset($_SESSION['id'])){
    ...
    switch($action)
    {
        ...
        DATE_FORMAT(membre_inscrit, \'%d/%m/%Y\') AS membre_inscrit_fr, membre_localisation
        ...
        echo'<h1>Profil de '.stripslashes(htmlspecialchars($data['membre_pseudo']));
        //Bouton pour modifier profil
        if($membre == $_SESSION['id']){     
          echo '<a href="voirprofil.php?m='
            .$_SESSION['id'].'&amp;action=modifier" class="button">Modifier profil</a></h1>';
        }
        else echo '</h1>';
        ...
        <strong>' .$data['membre_inscrit_fr'] .'</strong>  
        ...
        if (empty($_POST['sent'])) // Si on la variable est vide, on peut considérer qu'on est sur la page de formulaire
        {
            ...
            membre_siteweb, membre_signature, membre_localisation,
            membre_avatar
            ...
            <p>
            <input type="submit" style="font-size:1rem;" value="Valider les modifications" />
        ...
        else //Sinon on est dans la page de traitement
        {
            ...
            if ($i == 0) // : Pas d'erreur
            {
                ...
                //On peut choisir de supprimer l'avatar
                ...
                WHERE membre_id=:id');
                $query->bindValue(':mdp',password_hash($pass,PASSWORD_DEFAULT),PDO::PARAM_STR);
                $query->bindValue(':mail',$email,PDO::PARAM_STR);
                $query->bindValue(':website',$website,PDO::PARAM_STR);
                $query->bindValue(':sign',$signature,PDO::PARAM_STR);
                $query->bindValue(':loc',strtoupper($localisation),PDO::PARAM_STR);
                $query->bindValue(':id',$id,PDO::PARAM_INT);
                $query->execute();
                $query->CloseCursor();
            }
