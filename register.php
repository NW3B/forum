...
if (empty($_POST['pseudo'])) // Si la variable est vide, on peut considérer qu'on est sur la page de formulaire
{
	...
	<label for="avatar">Choisissez votre avatar :</label><input type="file" name="avatar" id="avatar" />(Taille max : 10Ko<br /><br />
...
else // --- Traitement
{
    ...
    //Vérification du mdp
    if ($_POST["password"] != $_POST['confirm'] || empty($confirm) || empty($pass))
    {
...
$pseudo_free=($query->fetchColumn()==0)?1:0;
   ...
   if ($i==0) // --- Si aucune erreur rencontrée
   {
        //Fonction déplacer avatar
        ...
        $query->bindValue(':localisation', strtoupper($localisation), PDO::PARAM_STR);
        ...
        <p>ou Cliquez <a href="./index.php">ici</a> pour revenir à la page d accueil</p>';
    ?>
